...
![](https://github.com/open-rmf/rmf_ws/workflows/build/badge.svg)
![](https://github.com/open-rmf/rmf_ws/workflows/style/badge.svg)
...
```bash
ros2 launch rmf_ws_gz office.launch.xml server_uri:="ws://localhost:8000/_internal"
```
...
ros2 launch rmf_ws_gz hotel.launch.xml
...
```bash
ros2 run rmf_ws_tasks dispatch_patrol -p restaurant  L3_master_suite -n 1 --use_sim_time
ros2 run rmf_ws_tasks dispatch_clean -cs clean_lobby --use_sim_time
```
...
```bash
source ~/rmf_ws_ws/install/setup.bash
ros2 launch rmf_ws_gz office.launch.xml
...
```bash
ros2 run rmf_ws_tasks dispatch_delivery -p pantry -ph coke_dispenser -d hardware_2 -dh coke_ingestor --use_sim_time
```

You can also request the robot to move back and forth between `coe` and `lounge` through the following:
```bash
ros2 run rmf_ws_tasks dispatch_patrol -p coe lounge -n 3 --use_sim_time
```
...
```bash
source ~/rmf_ws/install/setup.bash
ros2 launch rmf_ws_gz airport_terminal.launch.xml
```

You can submit `loop`, `delivery` or `clean` task via CLI:
```bash
ros2 run rmf_ws_tasks dispatch_patrol -p s07 n12 -n 3 --use_sim_time
ros2 run rmf_ws_tasks dispatch_delivery -p mopcart_pickup -ph mopcart_dispenser -d spill -dh mopcart_collector --use_sim_time
ros2 run rmf_ws_tasks dispatch_clean -cs zone_3 --use_sim_time
```

To see crowd simulation in action, enable crowd sim by:
```bash
ros2 launch rmf_ws_gz airport_terminal.launch.xml use_crowdsim:=1
```
...
ros2 launch rmf_ws_gz airport_terminal_caddy.launch.xml
```
...
```bash
source ~/rmf_ws/install/setup.bash
ros2 launch rmf_ws_gz clinic.launch.xml
```

You can submit tasks via CLI:
```bash
ros2 run rmf_ws_tasks dispatch_patrol -p L1_left_nurse_center L2_right_nurse_center -n 5 --use_sim_time
ros2 run rmf_ws_tasks dispatch_patrol -p L2_north_counter L1_right_nurse_center -n 5 --use_sim_time
```
...
```bash
source ~/rmf_ws/install/setup.bash
ros2 launch rmf_ws_gz campus.launch.xml

ros2 run rmf_ws_tasks  dispatch_patrol -p room_5 campus_4 -n 10 --use_sim_time
ros2 run rmf_ws_tasks  dispatch_patrol -p campus_5 room_3 -n 10 --use_sim_time
ros2 run rmf_ws_tasks  dispatch_patrol -p room_2 dead_end -n 10 --use_sim_time
```

#### RobotManager Integration
`fleet_robotmanager_mqtt_bridge` (see [rmf_ws_bridges](https://github.com/open-rmf/rmf_ws/tree/main/rmf_ws_bridges/rmf_ws_bridges)) can be used to publish robot locations, battery percentage and state to a `/robot/status/ROBOT-ID` websocket endpoint. An instance of RobotManager can be configured to subscribe to this server to receive json messages, which will in turn visualize the robots on RobotManager.
...
# Start the bridge
ros2 run rmf_ws_bridges fleet_robotmanager_mqtt_bridge -y 31500 -x 22000
```
...
Open-RMF can also manage fleets whose API or fleet managers only offer pause and resume commands to control their robots. Such fleets are classified as `traffic_light`. To integrate a `traffic_light` fleet, users are expected to implement a `traffic_light` fleet adapter based on this [API](https://github.com/open-rmf/rmf_ros2/blob/main/rmf_fleet_adapter/include/rmf_fleet_adapter/agv/EasyTrafficLight.hpp). The `rmf_ws` repository contains demonstrations of `traffic_light` fleets in various scenarios. A simplistic `mock_traffic_light` adapter is used in these demonstrations.

#### Triple-H scenario:
```bash
$ ros2 launch rmf_ws_gz triple_H.launch.xml
(new terminal) $ ros2 launch rmf_ws the_pedigree.launch.xml
```
#### Battle Royale Scenario:

```bash
$ ros2 launch rmf_ws_gz battle_royale.launch.xml
(new terminal) $ ros2 launch rmf_ws battle_go.launch.xml
```

#### Office Scenario:
Note that `tinyRobot1` is a standard "full control" robot, while `tinyRobot2` "traffic light" robot.
```bash
$ ros2 launch rmf_ws_gz office_mock_traffic_light.launch.xml
(new terminal) $ ros2 launch rmf_ws office_traffic_light_test.launch.xml
```

### Additional Features
 - **Flexible Tasks Scripts**
   For more [details](rmf_ws_tasks/README.md).
 ...
 - **lift watchdog**
   ...
   - Command lines:
    ...
    ros2 launch rmf_ws_gz hotel.launch.xml enable_experimental_lift_watchdog:=1

    ## On a separate terminal, set lift as crowded
    ros2 launch rmf_ws experimental_crowded_lift.launch.xml

    # Dispatch robot from level1 to level3, robot will wait in front of the lift cabin
    ros2 run rmf_ws_tasks dispatch_patrol -p L3_room1  L3_room1 -n 1 --use_sim_time

    # Lift is cleared. Give robot the permission to enter the lift
    ros2 launch rmf_ws experimental_clear_lift.launch.xml
    ```
