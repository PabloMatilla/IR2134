...
- **dispatch_patrol**
  ...
  ```
  ros2 run rmf_ws_tasks dispatch_patrol -F tinyRobot -R tinyRobot1 -p lounge -n 2 --use_sim_time
  ```

  or multiple places for the robot to travel to:
  ```
  ros2 run rmf_ws_tasks dispatch_patrol -p supplies pantry coe --use_sim_time
  ```
...
- **dispatch_clean**
  ...
  ```
  ros2 run rmf_ws_tasks dispatch_clean -cs clean_lobby -F cleanerBotA -R cleanerBotA_1 --use_sim_time
  ```
...
- **dispatch_action**
  ...
  ```
  ros2 run rmf_ws_tasks dispatch_action -F tinyRobot -R tinyRobot1 -a teleop -s coe --use_sim_time
  ```
  ...
  ```
  ros2 run rmf_ws_tasks dispatch_action -a teleop -s coe supplies pantry --use_sim_time
  ```
...
- **dispatch_delivery**
  ...
  ```bash
  ros2 run rmf_ws_tasks dispatch_delivery \
  -p pantry pantry \
...
- **cancel_task**
  ...
  ```
  ros2 run rmf_ws_tasks dispatch_patrol -p coe lounge --use_sim_time
  ```

  Then try cancel the submitted task with the id.
  ```bash
  ros2 run rmf_ws_tasks cancel_task -id patrol.dispatch-0
  ```
...
- **office_teleop_robot**
  ...
  ```
  ros2 launch rmf_ws office_teleop_robot.launch.xml
  ```
...
- **dispatch_go_to_place**
  ...
  ```
  ros2 run rmf_ws_tasks  dispatch_go_to_place -p lounge -o 105 --use_sim_time
  ```
...
- **wait_for_task_complete**
  ...
  ```
  ros2 run rmf_ws_tasks wait_for_task_complete -F tinyRobot -R tinyRobot1
  ```
